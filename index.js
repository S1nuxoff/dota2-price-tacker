...
const ITEMS_API_BASE_URL =
  "https://raw.githubusercontent.com/S1nuxoff/dota2-file-tracker";
...
async function getAllItemNames() {
  return Promise.all([
    fetch(`${ITEMS_API_BASE_URL}/main/static/items_game.txt.json`)
      .then((res) => res.json())
      .then((items) => {
        let prefabItems = [];
        for (let key in items) {
          if (items[key].prefab) {
            prefabItems.push(items[key]);
          }
        }
        prefabItems.sort((a, b) => a.prefab.localeCompare(b.prefab));
        return prefabItems.map((item) => item.name).filter(Boolean);
      }),
...
async function fetchPrice(name) {
  return new Promise((resolve, reject) => {
    ...
    community.request.get(
      `${MARKET_BASE_URL}/pricehistory/?appid=570&market_hash_name=${encodeURIComponent(
        name
      )}`,
      (err, res) => {
        if (err) {
